...
public class LinearProbingHashST2<Key, Value> {
    ...
    private void resize(int capacity) {
        ...
        for (int i = 0; i < m; i++) {
            if (keys[i] != null && marks[i]) {
                temp.put(keys[i], vals[i], marks[i]);
            }
        }
        keys = temp.keys;
        vals = temp.vals;
     ...
     * @throws IllegalArgumentException if {@code key} is {@code null}
     */
    public void put(Key key, Value val, boolean marked) {
    ...
    /*
    public boolean get1(Key key) {
        if (key == null) throw new IllegalArgumentException("argument to get() is null");

        for (int i = hash(key); keys[i] != null; i = (i + 1) % m) {
            if (keys[i].equals(key) && (marks[i] == true)) {
                return marks[i];
	    }   
        }
        return false;
    ...
    */
	...
	i = (i + 1) % m;
	    ...
	    boolean markedToRehash = marks[i];
            keys[i] = null;
            vals[i] = null;
	    marks[i] = null;
            n--;
            put(keyToRehash, valToRehash, false);
            i = (i + 1) % m;
        }
	*/
    ...
    public Iterable<Key> keys() {
        ...
        for (int i = 0; i < m; i++)
            if (keys[i] != null && marks[i]) queue.enqueue(keys[i]);
        return queue;
    }
    ...
    private boolean check() {
        ...
        if (m < 2*n1) {
            System.err.println("Hash table size m = " + m + "; array size n = " + n1);
            return false;
        }
	...
	for (int i = 0; i < keys.length; i++) {
	    st.put(keys[i], i, true);
	}
	
	for (String s : st.keys()) {
            StdOut.println(s + " " + st.get(s)); // + " " + st.get1(s));
	}

	StdOut.println(st.size() + " " + st.n1 + " " + st.m);
